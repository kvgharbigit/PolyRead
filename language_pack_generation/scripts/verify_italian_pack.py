#!/usr/bin/env python3
"""
Verify Italian pack specifically matches German/Spanish behavior
"""
...
def compare_with_existing_packs():
    """Compare Italian pack with German, Spanish, and French"""
    ...
    packs = {
        ...
        'Italian': 'assets/language_packs/it-en.sqlite'
    }
...
def main():
    """Verify Italian pack structure and behavior"""
    print("🇮🇹 ITALIAN LANGUAGE PACK VERIFICATION")
    print("=" * 80)
    
    italian_db = Path("assets/language_packs/it-en.sqlite")
    
    if not italian_db.exists():
        print(f"❌ Italian pack not found: {italian_db}")
        return False
    
    # Verify structure
    structure_ok = verify_pack_structure("Italian", italian_db)
    
    if not structure_ok:
        print("❌ Italian pack structure verification failed")
        return False
    ...
    print("=" * 50)
    print("✅ Schema: Matches German/Spanish/French structure")
    ...
    print(f"\n🎉 Italian↔English pack is ready for deployment!")
    print(f"📦 Total entries: {comparison.get('Italian', {}).get('total', 0):,}")
